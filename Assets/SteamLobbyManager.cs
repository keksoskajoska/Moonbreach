...
public class SteamLobby : MonoBehaviour
{
    ...
    private void OnLobbyCreated(LobbyCreated_t callback)
    {
        ...
        if (Menu.Instance.DidPlayerNameLobby)
        {
            SteamMatchmaking.SetLobbyData(
            new CSteamID(callback.m_ulSteamIDLobby),
            "name",
            Menu.Instance.LobbyName);
            Menu.Instance.DidPlayerNameLobby = false;
        }
        else
        {
            SteamMatchmaking.SetLobbyData(
    ...
    private void OnLobbyEntered(LobbyEnter_t callback)
    {
        ...
        networkManager.GetComponent<FishySteamworks.FishySteamworks>().SetClientAddress(hostAddress);
        networkManager.ClientManager.StartConnection();
        lobbyIDS.Clear();
        if (GameObject.Find("MainMenu"))
        {
            if (Menu.Instance.CurrentLobbies.Count > 0)
                Menu.Instance.DestroyOldLobbyListItems();
        }
    }
    ...
    {
        Menu.Instance.DisplayLobbies(lobbyIDS, result);
    }
